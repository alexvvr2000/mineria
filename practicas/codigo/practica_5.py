...
def generar_modelo_lineal(dataframe: pd.DataFrame, carpeta_destino: Path) -> list[Path]:
    ...
    with open(carpeta_destino / "modelo_lineal.txt", "w") as archivo:
        archivo.write("Coeficientes del modelo:\n")
        for columna, coef in zip(X.columns, modelo.coef_):
            archivo.write(f"{columna}: {coef}\n")
    ...
    for columna in X.columns:
        ...
        plt.title(f"Comparación de {columna} contra total de heridos")
        plt.legend()
